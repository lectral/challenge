...
test.describe("Google Maps Search", () => {
  ...
  test("should show information in sidebar after performing search", async ({
    page,
    context,
    ui,
  }) => {
  ...
  test("should show search entry as destination after clicking directions", async ({
    page,
    context,
    ui,
  }) => {
  ...
  test("should display proper error message when searching for non-existing location", async ({
    ui,
  }) => {
    ...
    await ui.maps.sidebarSummary.assertErrorLocationNotFound(
      nonExistingLocation
    );
  });

  test("should show list of search recommendations on partial search", async ({
    ui,
  }) => {
  ...
  test("should show list of searches for non-obvious location", async ({
    ui,
  }) => {
  ...
  test("should properly handle lowercase, upper and mixed case searches", async ({
    ui,
  }) => {
  ...
  test("should properly handle search with diacritics", async ({ ui }) => {
    const search = "Łódź";
    await ui.maps.searchBar.fill(search);
    await ui.maps.searchBar.triggerSearchByClick();
    await ui.maps.sidebarSummary.assertHeadlineText(search);
  });
});
